...
var checkNative = function() {
	...
	try {
		if (! isNativeWorking) {
			...
			if (limit) {
				alert('Firefox Extension is not installed');
				
				if(returnToURL == '') {
					//window.location.assign(hostURL + '/ipm-web/pages/public/installCSSignerFirefox.jsf?returnUrl=' + hostURL + 'ipm-web');
				} else {
					//window.location.assign(hostURL + '/ipm-web/pages/public/installCSSignerFirefox.jsf?returnUrl=' + returnToURL);
				}
