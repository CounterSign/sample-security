...
function checkRequiredFilesMicrosoftInstalled(urlToDownload) {
	if(document.CSSignerApp.checkRequiredFilesMicrosoftInstalled("urlToDownload")==true) {
		alert("Arquivos necessários para o InternetExplorer instalados com sucesso!");
	} else {
		alert("Arquivos necessários para o InternetExplorer não estão instalados na máquina.");
	}
}
	...
	if(thumbprint==undefined || thumbprint==''){
		...
		if(document.CSSignerApp.selectCertificates("Microsoft")) {
			...
			while(document.CSSignerApp.hasNext()) {
				...
				var validate = document.CSSignerApp.getNotAfter();
				if(i ==0) {
					text += "<option selected='selected' title='/gcont-web/pages/static/css/themes/default/images/certificate.png' value='" + thumbprintLocal + "'>" + cn + ";" + validate  + ";" + ac + "</option>";
				} else {
					text += "<option title='/gcont-web/pages/static/css/themes/default/images/certificate.png' value='" + thumbprintLocal +  "'>" + cn + ";" + validate + ";" + ac + "</option>";
				}
				i++;
			}
			
			text +="</select>";
			//text += "<br/><button id='selectBtn' onclick='selectCertificate();'>Ok</button>";
			text += "</div><br/>";
			//text +="<button class='btn_masterBigLoginC tam100 direita' id='fechar' onclick='jQuery.unblockUI()'>Cancelar</button>";			
...
function decipherPkcs1B64(cipheredData, thumb) {
	thumbprint = thumb;
	if(thumbprint == undefined || thumbprint == '') {
		alert("Erro na decriptografia: Certificado não foi selecionado");
		return '';
	}
